...
throw new Error(response.data.message)
 ...
 function displayDownloadHistory(data){
  document.getElementById('message').innerHTML+=`<br><br>Dowload History <br><hr>`
  for(let i=0;i<data.length;i++){
    document.getElementById().innerHTML+= `<br> url:<a href =${data[i].url}> report${i+1}</a> <br> Download at ${data[i].createdAt}<br><br> <hr>`
  }
 }
