...
const expenseDetail={                                                                                                                   // store the users in the right format..
 ...
 function showpage({
  ...
  if(haspreviouspage){
    const bn2=document.createElement("button")
    bn2.type='button'
    bn2.innerHTML=previouspage
    bn2.addEventListener('click', ()=>getExpenses(previouspage))
    pagination.appendChild(bn2)
  
  }
  const bn1=document.createElement('button')
  bn1.type='button'
  bn1.innerHTML=`<h3>${currentpage}</h3>`
  bn1.addEventListener('click', ()=>getExpenses(currentpage))

  if(hasnextpage){
    const bn3=document.createElement("button")
    bn3.type='button'
    bn3.innerHTML=nextpage
    bn3.addEventListener('click', ()=>getExpenses(nextpage))
    pagination.appendChild(bn3)
